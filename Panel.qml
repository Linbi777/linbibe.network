...
Panel {
  ...
  // Shibumi-style pages on the Wi-Fi icon: Wi-Fi stays the default, Amnezia
  // and WireGuard are independent toggles and can stay up together.
  ...
  KeyboardPanel {
    ...
    Column {
      ...
      Row {
        ...
        Button {
          width: connectivityTabs.cellWidth
          text: "Wi-Fi"
          ...
          bordered: true
          active: root.connectivityTab === "wifi"
          onClicked: root.connectivityTab = "wifi"
        }
        ...
        Button {
          ...
          verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
          bordered: true
          active: root.connectivityTab === "amnezia"
          onClicked: root.connectivityTab = "amnezia"
        }
        ...
        Button {
          ...
          verticalPadding: Style.spacing.controlPaddingY + Style.space(2)
          bordered: true
          active: root.connectivityTab === "wireguard"
          onClicked: root.connectivityTab = "wireguard"
        }
      }

      VpnTunnels {
        width: parent.width
        visible: root.connectivityTab === "amnezia" || root.connectivityTab === "wireguard"
        hostPanel: root
        page: root.connectivityTab
        pageActive: root.opened && (root.connectivityTab === "amnezia" || root.connectivityTab === "wireguard")
      }

      // Connection details: transfer metrics first, then IP/Gateway.
      Column {
        visible: root.connectivityTab === "wifi" && !!root.info.iface
        width: parent.width
      ...
      PanelSeparator {
        visible: root.connectivityTab === "wifi" && root.canSelectBand
        foreground: root.bar.foreground
      }
      ...
      PanelSeparator {
        visible: root.connectivityTab === "wifi"
        foreground: root.bar.foreground
      }
      ...
      PanelSeparator {
        visible: root.connectivityTab === "wifi" && root.wifiStationAvailable
        foreground: root.bar.foreground
      }

      PanelSectionHeader {
        visible: root.connectivityTab === "wifi" && root.wifiStationAvailable && root.scanning
        text: "SCANNING WI-FI…"
        foreground: root.bar.foreground
        fontFamily: root.bar.fontFamily
      }
